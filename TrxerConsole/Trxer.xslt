                  ...
                  <tr>
                    ...
                    <td>
                      ...
                      <xsl:for-each select="$nameSet">
						  <xsl:choose>
							  <!-- Render anchor tag if the owner string contains 'http' -->
							  <xsl:when test="contains(@name, 'http')">
								  <a href="{@name}" target="_blank">
									  <xsl:call-template name="LinkIcon"></xsl:call-template>
								  </a>
							  </xsl:when>
							  <!-- Render plain text -->
							  <xsl:otherwise>
...
<xsl:template name="LinkIcon">
	<svg xmlns="http://www.w3.org/2000/svg" width="22px" height="22px" viewBox="0 0 15 15" fill="none">
